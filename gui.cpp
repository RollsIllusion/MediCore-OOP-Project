...
// ---------------------------------------------------------------
// ScrollableList copy constructor — deep-copies the row arrays.
// Required because the default (compiler-generated) copy would
// shallow-copy rows_ / isHeader_, causing a double-free when the
// temporary created by list_ = ScrollableList(...) is destroyed.
// ---------------------------------------------------------------
ScrollableList::ScrollableList(const ScrollableList& o)
    : bounds_(o.bounds_), theme_(o.theme_), maxRows_(o.maxRows_),
    rowCount_(o.rowCount_), selectedRow_(o.selectedRow_),
    scrollOffset_(o.scrollOffset_), rowHeight_(o.rowHeight_),
    background_(o.background_), scrollBarShape_(o.scrollBarShape_)
{
    if (maxRows_ > 0) {
        rows_ = new char* [maxRows_];
        isHeader_ = new bool[maxRows_];
        for (int i = 0; i < maxRows_; ++i) {
            rows_[i] = new char[256];
            safeCopy(rows_[i], o.rows_[i], 256);
            isHeader_[i] = o.isHeader_[i];
        }
    }
    else {
        rows_ = nullptr;
...
// ---------------------------------------------------------------
// ScrollableList copy assignment — same deep-copy logic.
// ---------------------------------------------------------------
ScrollableList& ScrollableList::operator=(const ScrollableList& o) {
    if (this == &o) return *this;
    ...
    selectedRow_ = o.selectedRow_;
    scrollOffset_ = o.scrollOffset_;
    rowHeight_ = o.rowHeight_;
    background_ = o.background_;
    scrollBarShape_ = o.scrollBarShape_;
...
void BookAppointmentPanel::handleEvent(const sf::Event& ev, sf::Vector2f mpos,
    ...
    case 0:
        ...
        inputSpec_.update(mpos, clicked);
    ...
    case 1:
        ...
        doctorList_.update(mpos, clicked);
    ...
    case 2:
        ...
        inputDate_.update(mpos, clicked);
...
void ViewMedicalRecordsPanel::handleEvent(const sf::Event& ev, sf::Vector2f mpos,
    bool clicked, bool mouseDown) {
    (void)clicked;
    list_.handleEvent(ev); list_.update(mpos, clicked);
    btnBack_.update(mpos, mouseDown);
    if (btnBack_.isClicked()) next_ = Screen::PatientMenu;
}
...
void TopUpPanel::handleEvent(const sf::Event& ev, sf::Vector2f mpos,
    bool clicked, bool mouseDown) {
    (void)clicked;
    inputAmount_.handleEvent(ev); inputAmount_.update(mpos, clicked);
...
void ViewTodayPanel::handleEvent(const sf::Event& ev, sf::Vector2f mpos,
    bool clicked, bool mouseDown) {
    (void)clicked; list_.handleEvent(ev); list_.update(mpos, clicked);
    btnBack_.update(mpos, mouseDown);
...
void WritePrescriptionPanel::handleEvent(const sf::Event& ev, sf::Vector2f mpos,
    ...
    (void)clicked;
    if (step_ == 0) {
        inputApptId_.handleEvent(ev); inputApptId_.update(mpos, clicked);
        ...
        inputMedicines_.handleEvent(ev); inputMedicines_.update(mpos, clicked);
        inputNotes_.handleEvent(ev); inputNotes_.update(mpos, clicked);
...
void ViewPatientHistoryPanel::handleEvent(const sf::Event& ev, sf::Vector2f mpos,
    bool clicked, bool mouseDown) {
    (void)clicked;
    inputPatientId_.handleEvent(ev); inputPatientId_.update(mpos, clicked);
    list_.handleEvent(ev); list_.update(mpos, clicked);
...
void AddDoctorPanel::handleEvent(const sf::Event& ev, sf::Vector2f mpos,
    bool clicked, bool mouseDown) {
    (void)clicked;
    inputName_.handleEvent(ev); inputName_.update(mpos, clicked);
    inputSpec_.handleEvent(ev); inputSpec_.update(mpos, clicked);
    inputContact_.handleEvent(ev); inputContact_.update(mpos, clicked);
    inputPassword_.handleEvent(ev); inputPassword_.update(mpos, clicked);
    inputFee_.handleEvent(ev); inputFee_.update(mpos, clicked);
...
void MessagePanel::onEnter() {
    ...
    // Reconstruct button to clear any stale pressed_ state from the previous panel's click
    sf::FloatRect b = btnOk_.getBounds();
    btnOk_ = Button({ b.left, b.top, b.width, b.height }, "OK", *theme_);
}
